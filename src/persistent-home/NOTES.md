...
Paths must be canonical relative directory paths. Absolute paths, `~`, `.` or `..` components, trailing slashes, and overlapping paths are rejected. File paths are not supported.

For a non-root `remoteUser`, passwordless `sudo` is required so the Feature can correct ownership of the volume after the container is created and any remote-user UID/GID update has been applied.
